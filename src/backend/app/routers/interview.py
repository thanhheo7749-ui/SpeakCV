...
def clean_and_parse_json(raw_text: str):
    """Hàm dọn dẹp các ký tự rác do AI sinh ra trước khi parse JSON"""
    ...
    try:
        ...
        print(f"Chuỗi thô sau khi dọn: {cleaned}")
        raise ValueError("AI trả về định dạng JSON không hợp lệ.")
...
async def transcribe_audio(
    ...
    try:
        ...
        if not groq_key: return {"text": "Lỗi: Chưa cài đặt GROQ_API_KEY"}
...
async def chat(request: models.ChatRequest, http_req: Request, db: Session = Depends(get_db)):
    ...
    if not auth_header or auth_header == "Bearer null":
        ...
        if current_count >= 20:
            print(f"🚫 Đã chặn IP {client_ip} vì hết 20 lượt.")
...
async def update_interview_history(
    ...
    if not history:
        raise HTTPException(status_code=404, detail="Không tìm thấy lịch sử phỏng vấn.")
